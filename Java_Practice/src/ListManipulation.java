...
public class ListManipulation {
    public static void main(String args[]) {
        ...
        findMaxandMin();
        filterFromArrayList();
        mergeTwoUnsortedArrayIntoSingleArray();
        sortlistofStringInIncreasingOrderBytheirLength();
    ...
    public static void mergeTwoUnsortedArrayIntoSingleArray(){
        List<Integer> li1 = List.of(1,2,4,3,8,5,9);
        List<Integer> li2 = List.of(10,11,55,665,8,6);
        List<List<Integer>> li3 = List.of(li1, li2);

        List<Integer> response =
        li3.stream().flatMap(x-> x.stream())
                .sorted().collect(Collectors.toList());
        System.out.println("Merge List "+response);

        System.out.println("Merge List With stream Concat"+
        Stream.concat(li1.stream(),li2.stream()).sorted().collect(Collectors.toList()));

    }

    //How to sort an List of string in increasing order by their length in Java 8?
    public static void sortlistofStringInIncreasingOrderBytheirLength() {
        List<String> list = Arrays.asList("Pomogranate", "Mango", "AppleCyder","ant","Papaya");
        //Ascending Orer
        System.out.println("List is " + list);
        System.out.println(" Ascending Order By String length " + list.stream().sorted(Comparator.comparing(String::length)).collect(Collectors.toList()));
        System.out.println(" Descending Order By String length " + list.stream().sorted(Comparator.comparingInt(String::length).reversed()).collect(Collectors.toList()));
    }
}